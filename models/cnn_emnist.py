...
class ModelCNNEmnist(ModelCNNAbstract):
    ...
    def create_graph(self, learning_rate=None):
        ...
        self.y_ = tf.placeholder(tf.float32, shape=[None, 62])
        ...
        self.W_fc2 = weight_variable([256, 62])
        self.b_fc2 = bias_variable([62])
        ...
        self.cross_entropy = tf.reduce_mean(-tf.reduce_sum(self.y_ * tf.log(self.y), reduction_indices=[1]))  # tf.maximum(self.y,1e-20) to avoid nan
        # self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=None)  # Learning rate should not be needed
